...
public class WayRoom4 : MonoBehaviour {
    ...
    public void Check(Ray ray, float rotation)
    {
        ...
        {
            if ((hit.transform.name.StartsWith("1-Way") && (hit.transform.rotation != transform.rotation * Quaternion.Euler(0f, 180f, 0f) * Quaternion.Euler(0f, rotation, 0f)))
